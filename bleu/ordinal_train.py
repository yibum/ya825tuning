...
def learning_margin(cand_ranking, feat_weight, order_margin):
    ranking = []
    ...
    margin = ranking[0][2] - ranking[10][2]
    return margin
...
def tuning_feature_weight(golden_ranking, feat_weight, order_margin, margin):
    u_vec = []
    for i in xrange(0, len(golden_ranking)):
        u_vec.append(0.0)
    for i in xrange(0, len(golden_ranking)-order_margin):
        for j in xrange(i+order_margin, len(golden_ranking)):
            (sen, top_feat, score) = golden_ranking[i]
            (sen, bottom_feat, score) = golden_ranking[j]
            top_score = cal_feat_score(top_feat, feat_weight)
            bottom_score = cal_feat_score(bottom_feat, feat_weight)
            if top_score - bottom_score < margin*uneven_function(float(i+1), float(j+1)):
                u_vec[i] += uneven_function(float(i+1), float(j+1))
                u_vec[i+order_margin] -= uneven_function(float(i+1), float(j+1))
...
def uneven_function(p, q):
    result = float(1/p) - float(1/q)
    return result
...
if __name__ == '__main__':
    ...
    for itr in xrange(0,10):
        for i in xrange(0, len(candidates_table)):
            bleu_file = "bleu_result/bleu_sentence"+str(i+1)
            margin = learning_margin(candidates_table[i], feature_weight, 10)
            tuning_feature_weight(bleu_ranking[i], feature_weight, 10, margin)
